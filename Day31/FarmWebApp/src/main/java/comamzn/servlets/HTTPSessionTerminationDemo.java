...
public class HTTPSessionTerminationDemo extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// demo removing session attribute
		httpSession.removeAttribute("Fav-Cloth");
		if(httpSession!=null) {
			// check if there is anything in the session object'
				pw.print("<br/>Following items were found in your session after removing \"Fav-Cloth\".");
				
				attrNames = httpSession.getAttributeNames();
		...
		pw.print("<br/>Terminating your http session.");
		httpSession.invalidate(); // Terminating the http session
		pw.print("<br/>Hey there is a http session "
				+ "for current interaction with the web app and now terminated");
		}
		else
